...
describe('Rute cart', () => {
  ...
  // Ruleaza testele admin doar daca avem token valid
  if (authToken) {
         it('GET /api/admin/products cu token valid ar trebui sa returneze produsele', async () => {
         const response = await request(app)
         ...
         const newProduct = {
          ...
          it('PUT /api/admin/products/:id ar trebui sa actualizeze un produs', async () => {
          const updates = { title: 'Updated Title' };
          const response = await request(app)
           .put('/api/admin/products/1')
           .set('Authorization', `Bearer ${authToken}`)
           .send(updates);
      // Accepta orice status intre 200, 404, 500
...
describe('Rute autentificare', () => {
    ...
    // Acceptă atât succes cât și failure – important este ca răspunde
         ...
         it('POST /api/admin/login ar trebui sa respinga credientialele        invalide', async () => {
         const response = await request(app)
          .post('/api/admin/login')
          .send({
